...
for r in range(4, (ws.max_row or 0) + 1):
    ...
    notes[note[:50] or "(empty)"] = notes.get(note[:50] or "(empty)", 0) + 1
    if not any(x in low for x in ("досроч", "доссроч", "выкуплен")):
        continue
    init += num(ws, r, 6)
    bal += num(ws, r, 9)
    val += num(ws, r, 10)
    n += 1
print("early filtered n", n, "init", round(init, 2), "bal", round(bal, 2), "val", round(val, 2))
